...
@Component({
  selector: 'app-co-datepicker-range',
  templateUrl: './current-order-datepicker-range.html',
  styles: [`
    .custom-day {
      text-align: center;
...
export class CurrentOrderDatepickerRangeComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.ranges = this.currentOrderService.ranges;
    this.currentOrderService.rangesChanged.subscribe(
      (ranges: RangeComponent[]) => {
        this.ranges = ranges;
      });

    this.costumeModel = this.currentOrderService.costumeModel;
    this.currentOrderService.costumeModelChanged.subscribe(
      (costumeModel: CostumeModelComponent) => {
